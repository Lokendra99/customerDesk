...
eventEmitter.on('sendMail' , function(data){
    ...
    var transporter = nodemailer.createTransport({
        service: 'Gmail',
        auth: {
          user: 'customerdesk999@gmail.com',
          pass: 'customerdesk123'
        }
    });

    var querymsg=null;
    if(data.email=='customerdeskhead@gmail.com'){
      querymsg='You have recieved a reply '+'for ticket Number '
    }
    else{
      querymsg='You have recieved a answer from admin for ticket Number'
    }

    var mailOptions = {
        from: 'CustomerDesk <customerdesk999@gmail.com>',
        to: data.email,
        subject: 'CustomerDesk Notification',
        text: 'Answer received for ticket number : '+ data.ticketNumber,
        html: '<h1>Hello '+data.name+'</h1><br><h2>'+querymsg+data.ticketNumber+'</h2>'
    }
            ...
            result.save(function(error){
                if(error){
                   ...
                   if(newMessage.sender!='Admin'){
                    eventEmitter.emit('sendMail' , {ticketNumber : data , name: 'Admin' ,
                    email:'customerdeskhead@gmail.com'});
                   }
                   else{

                     eventEmitter.emit('sendMail' , {ticketNumber : data , name: name ,
                       email: email});
                   }
